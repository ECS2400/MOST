...
export function MediationV2FirstDealScreen({
  content,
  actions,
  busy,
  waitingForPartner,
  ...
  const dealText = readString(content.dealText);

  return (
    <View style={styles.wrap}>
      <Text style={styles.body}>{dealText || '—'}</Text>
      {waitingForPartner ? (
        <Text style={styles.wait}>Oczekiwanie na głos partnera…</Text>
      ) : null}
