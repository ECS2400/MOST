...
export function MediationV2LessonScreen({
  content,
  actions,
  busy,
  waitingForPartner,
  ...
  const observation = readString(content.observation);
  const lesson = readString(content.lesson);

  return (
    <View style={styles.wrap}>
      {observation ? <Text style={styles.obs}>{observation}</Text> : null}
      <Text style={styles.body}>{lesson || '—'}</Text>
      {waitingForPartner ? (
